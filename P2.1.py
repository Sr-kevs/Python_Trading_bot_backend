...
def order(symbol, side, quantity, order_type=ORDER_TYPE_MARKET):
    try:
        print("sending order")
        order = client.futures_create_order(symbol=symbol, side=side, positionSide = long , quantity=quantity, type=order_type)
        print(order)
...
def on_close(ws):
    ...
    time.sleep(60)
    subprocess.run ([sys.executable,'C:/Users/kevin/Documents/Inversion/Bit trading/FuturesBot3/P2.1.py'] ) 
    sys.exit()       
...
def on_message(ws, message):
    ...
    #variables for counting how many one minute closing candles are recevive by socket
    #once a set parameter is set do an action
    #conf = 1 
    #
    #if is_candle_closed:
    ...
    #    if (d[conf]) > 15:
    #        subprocess.run ( [sys.executable, 'C:/Users/kevin/Documents/Inversion/Bit trading/FuturesBot3/P2.1.py' ] ) and sys.exit()


    
    #lines of code to capture inmeadiate last closed price and save temporary in a list   
    if close:
        ...
        if len(closes):
            ...
            if last_closed > dsup10  :
                ...
                order = client.futures_create_order(
                    ...
                    quantity= round(amount_future_contracts_long ) )
                print (order)  
                ...
                #subprocess.run ([sys.executable, 'C:/Users/kevin/Documents/Inversion/Bit trading/FuturesBot3/P3.py' ] ) 
                sys.exit()
        ...
        if len(closes):
            ...
            if last_closed < dsup01:
                print("Reached 2 percent this code is not needed no more")
                subprocess.run ([sys.executable, 'C:/Users/kevin/Documents/Inversion/Bit trading/FuturesBot3/sellupmarket2.py' ] ) 
                sys.exit()
