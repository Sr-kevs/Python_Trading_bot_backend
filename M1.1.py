...
def on_close(ws):
    ...
    time.sleep(60)
    subprocess.run ([sys.executable,'filepath/M1.1.py'] ) 
    sys.exit()
...
def on_message(ws, message):
    ...
    #    if (d[conf]) > 15:
    #        subprocess.run ( [sys.executable, 'filepath/M1.1.py' ] ) and sys.exit()
        ...
        if len(closes):
            ...
            if last_closed < dsupm08 :       
                ...
                order = client.futures_create_order(
                    ...
                    quantity= round( ( (amount_future_contracts_short*-1)*.4))
                    )
                print(order)
                #once condition is met and action triggerred automates to intiate next script
                subprocess.run([sys.executable ,'filepath/DOWNSELLUSDT00.py']) 
                sys.exit()
