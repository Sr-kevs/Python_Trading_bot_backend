...
def on_close(ws):
    ...
    time.sleep(60)
    subprocess.run ([sys.executable,'filepath/P1.4.py'] ) 
    sys.exit()       
...
def on_message(ws, message):
    ...
    #    if (d[conf]) > 15 :
    #        subprocess.run ( [sys.executable, 'filepath/P1.4.py' ] ) and sys.exit()
    ...
    if close:
        ...
        if len(closes):
            ...
            if last_closed > dsup05  :
                #last_closed > dsup10   :                
                #print("Sold all down token @ 1 percent up")
                ##script for reproducing a sound when action is triggered 
                #playsound('filepath/cashingmachine.mp3')
                subprocess.run ( [sys.executable, 'filepath/DOWNBUYUSDT00.py' ] ) 
                sys.exit()
