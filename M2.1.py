...
def on_close(ws):
    ...
    time.sleep(60)
    subprocess.run ([sys.executable,'filepath/M2.1.py'] ) 
    sys.exit()    
...
def on_message(ws, message):
    ...
    #    if (d[conf]) > 15:
    #        subprocess.run ( [sys.executable, 'filepath/M2.1.py' ] ) and sys.exit()
        ...
        if len(closes):
            ...
            if last_closed > dsup00:
                if in_position:
                    ...
                    subprocess.run ([sys.executable, 'filepath/M1.3.py' ] )
