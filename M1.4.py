...
def on_close(ws):
    ...
    time.sleep(60)
    subprocess.run ([sys.executable,'filepath/M1.4.py'] ) 
    sys.exit()
...
def on_message(ws, message):
    ...
    #sconf = 1 
    #s
    #sif is_candle_closed:
    #s    print("Is candle closed?? {}".format(conf))
    #s    confs.append(float(conf))
    #s    d = (Counter(confs))
    #s    print("It have passed {} minutes".format (d[conf]))
    #s    if (d[conf]) > 15:
    #s        subprocess.run ( [sys.executable, 'filepath/M1.4.py' ] ) and sys.exit()
