...
def on_message(ws, message):
    ...
    if close:
        ...
        #this code was for testng if could gain acces from API to wallet and show balance from there
        #balance3 = client.get_asset_balance(asset=asset1)
        #wallet_value3 = int (float ( balance3 ['free'] ) )
        #print(" Last wallet value of UP token {}".format(  round (wallet_value3 ) ) )
        
        #balance4 = client.get_asset_balance(asset=asset2)
        #wallet_value4 = int (float ( balance4 ['free'] ) )
        #print(" Last wallet value of UP token {}".format(  round (wallet_value4 ) ) )
        ...
        if len(closes):
            ...
            if last_closed > dsup05 :
                ...
                #    quantity= round(amount_future_contracts_long*.4 ) )
                #print (order)
                #once condition is met and action triggerred automates to intiate next script
                subprocess.run ([sys.executable,'filepath/UPSELLUSDT00.py'] ) 
                sys.exit()
